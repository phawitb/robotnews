...
## Required English edition

Each item must also include `english`: `{ "title": "English headline", "summary": "English summary", "sections": [{"heading":"English heading","paragraphs":["Detailed English text..."]}], "images": [], "videos": [] }`. Supply at least three sections, each over 100 characters, with equivalent facts to the Thai article. `images` must match `media.images` in length and order with `{url, alt, caption}` in English. `videos` must match `media.videos` in length and order with `{url, title, note}` in English; a link fallback needs an explanatory note. URLs must exactly match the shared media records. Use empty lists only when the source has no media. Playback URLs and files are shared; translate captions and titles, not the media itself. Import stores this in content/english.json and the build creates /en and /en/articles/<id>.html.
